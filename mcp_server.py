...
TAVILY_MONTHLY_FREE = 1000
TAVILY_CAP = 950  # soft cap — leaves 50/mo headroom on Tavily
MAX_CALL_LOG = 500  # keep most-recent N calls in the audit log
...
def _empty_usage(month: str) -> dict:
    return {
        ...
        "tavily": {
            "count": 0,
            "errors": 0,
            "free_quota": TAVILY_MONTHLY_FREE,
            "soft_cap": TAVILY_CAP,
        },
...
def _load_usage() -> dict:
    ...
    if data.get("month") != month:
        return _empty_usage(month)
    data.setdefault("tavily", {"count": 0, "errors": 0})
    data.setdefault("duckduckgo", {"count": 0, "errors": 0})
    data["tavily"].setdefault("free_quota", TAVILY_MONTHLY_FREE)
    data["tavily"].setdefault("soft_cap", TAVILY_CAP)
...
def _log_call(provider: str, query: str, num_results: int, status: str) -> None:
    """Append one row to the per-call audit log and bump the aggregate counter."""
    with _usage_lock:
        data = _load_usage()
        data[provider]["count"] = data[provider].get("count", 0) + 1
        if status != "ok":
            data[provider]["errors"] = data[provider].get("errors", 0) + 1
        data["calls"].append({
        ...
        if len(data["calls"]) > MAX_CALL_LOG:
            data["calls"] = data["calls"][-MAX_CALL_LOG:]
        _save_usage(data)


def _under_cap(provider: str) -> bool:
    """True if we're still under the soft cap and can call Tavily."""
    return _load_usage()[provider]["count"] < TAVILY_CAP
...
def web_search(query: str, max_results: int = 5) -> list[dict]:
    ...
    if os.environ.get("TAVILY_API_KEY") and _under_cap("tavily"):
        try:
            ...
            if results:
                _log_call("tavily", query, len(results), "ok")
                return results
            _log_call("tavily", query, 0, "empty")
        except Exception as e:
            _log_call("tavily", query, 0, f"error:{type(e).__name__}")
    results = _ddg_search(query, max_results)
    _log_call("duckduckgo", query, len(results), "ok" if results else "empty")
    return results
